...
#define __FLOG_DECLARATION_MEMBER		RFileLogger __logger__ d;
...
LOCAL_C void LogToFile(const TDesC& fname, const TDesC& string)
{
	RFs fs;
	User::LeaveIfError(fs.Connect());
	CleanupClosePushL(fs);
	RFile file;		
	TInt err = file.Create(fs, fname, EFileWrite);
	if( err == KErrAlreadyExists) 
		file.Open(fs, fname, EFileWrite);
	TPtrC8 representation((TUint8*)(&string)->Ptr(), (&string)->Size());
	TInt pos;
	TInt nRetVal = file.Size(pos);
	nRetVal = file.Write(pos, representation);
	nRetVal = file.Flush();
	file.Close();		
...
LOCAL_C void LogToFile(const TDesC& fname, const TDesC& string)
{
	if (false)						// Just to avoid warning... "Declared but not used"
		LogToFile(fname, string);	// Just to avoid warning... "Declared but not used"
}
